...
// ─── Genera e imprime PDF usando ventana del navegador ───────────────────────
function generateStockPDF(title: string, items: Product[], tipo: 'bajo' | 'agotado') {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
    ...
    <style>
      ...
      h1{color:${color};font-size:22px;margin-bottom:4px}
      .sub{color:#64748b;font-size:13px;margin-bottom:24px}
      table{width:100%;border-collapse:collapse}
      th{background:${color};color:white;padding:10px 8px;text-align:left;font-size:12px;text-transform:uppercase;letter-spacing:.05em}
    ...
    <table>
      <thead><tr>
        <th>Producto</th><th>SKU</th><th style="text-align:center">Stock</th>
...
export default function InventarioTab({ products, onAddProduct, onEditProduct, onDeleteProduct }: InventarioTabProps) {
  ...
  // Foto desde galería del celular → base64
  ...
  return (
    <div id="inventario-container" className="space-y-5 pb-24">

      {/* 1. Stats Cards */}
      <section className="grid grid-cols-3 gap-3 mt-1">
        <div className="bg-white p-4 rounded-2xl border border-gray-100 flex flex-col gap-1 shadow-sm">
          <span className="text-xs font-bold text-gray-500 uppercase">Total Uds</span>
          <span className="text-2xl font-extrabold text-indigo-600">{totalStock.toLocaleString()}</span>
        </div>

        {/* Bajo Stock con PDF */}
        <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200/50 flex flex-col gap-1 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />
              <span className="text-xs font-bold text-amber-800 uppercase">Bajo Stock</span>
            </div>
            {lowStockItems.length > 0 && (
              <button
                onClick={() => generateStockPDF('⚠️ Reporte Bajo Stock', lowStockItems, 'bajo')}
                title="Descargar PDF"
        ...
        {/* Agotados con PDF */}
        <div className="bg-rose-50 p-4 rounded-2xl border border-rose-200/50 flex flex-col gap-1 shadow-sm">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
            ...
            {outOfStockItems.length > 0 && (
              <button
                onClick={() => generateStockPDF('🔴 Reporte Productos Agotados', outOfStockItems, 'agotado')}
                title="Descargar PDF"
      ...
      <section className="flex gap-2">
        <div className="relative flex-1 group">
          ...
          <input
            className="w-full pl-10 pr-4 py-3.5 bg-gray-50 border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-600/10 focus:border-indigo-600 focus:bg-white transition-all text-base placeholder:text-gray-400 h-13"
          ...
          />
          {search && (
            <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 font-bold text-lg">×</button>
          )}
        </div>
      </section>

      {/* 3. Categorías */}
      <section className="flex gap-2 overflow-x-auto -mx-4 px-4 py-1">
...
<section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{p.category}</span>
          <h3 className="font-bold text-gray-900 text-lg leading-tight truncate mt-0.5">{p.name}</h3>
          <p className="text-sm text-gray-500 font-medium mb-1">SKU: {p.sku}</p>
          <div className="flex items-center justify-between">
            <span className={`text-base font-bold ${isOutOfStock ? 'text-rose-600' : isLowStock ? 'text-amber-600' : 'text-gray-700'}`}>
              {isLowStock && '⚠️ '}{p.stock} uds
            </span>
            <span className="text-xl font-extrabold text-indigo-600">${p.price.toFixed(2)}</span>
          </div>
          <div className="flex justify-between mt-0.5">
            <span className="text-sm text-gray-500">Costo: ${p.cost.toFixed(2)}</span>
            <span className={`text-sm font-bold px-1.5 py-0.5 rounded ${marginPercent >= 30 ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-50 text-gray-500'}`}>
              +{marginPercent}%
            </span>
          </div>
        </div>
  ...
  {filteredProducts.length === 0 && (
    <div className="col-span-2 py-12 flex flex-col items-center justify-center text-center space-y-2">
      <PackageOpen className="w-12 h-12 stroke-1 text-gray-300" />
      <p className="text-base font-semibold text-gray-500">Ningún producto encontrado</p>
    </div>
  )}
</section>
              ...
              <div className="relative w-20 h-20 rounded-xl overflow-hidden bg-gray-50 shrink-0 border border-gray-100">
                ...
                {isOutOfStock && (
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                    <span className="bg-rose-600 text-white font-bold text-[9px] uppercase px-1.5 py-0.5 rounded">Agotado</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-bold text-gray-900 text-base leading-tight truncate mt-0.5">{p.name}</h3>
                <p className="text-xs text-gray-400 font-medium mb-1">SKU: {p.sku}</p>
                <div className="flex items-center justify-between">
                  <span className={`text-sm font-bold ${isOutOfStock ? 'text-rose-600' : isLowStock ? 'text-amber-600' : 'text-gray-700'}`}>
                    {isLowStock && '⚠️ '}{p.stock} uds
                  </span>
                  <span className="text-lg font-extrabold text-indigo-600">${p.price.toFixed(2)}</span>
                </div>
                <div className="flex justify-between mt-0.5">
                  <span className="text-xs text-gray-400">Costo: ${p.cost.toFixed(2)}</span>
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${marginPercent >= 30 ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-50 text-gray-500'}`}>
                    +{marginPercent}%
                  </span>
                </div>
              </div>
      ...
      {/* 6. Modal Agregar / Editar */}
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 px-0 sm:px-4">
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-sm overflow-hidden shadow-2xl flex flex-col max-h-[92vh]">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              {/* Vista previa de imagen + botones */}
                  ...
                  {PRESET_IMAGES.map((img, i) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-lg overflow-hidden border shrink-0 transition-all ${
                        formData.imageUrl === img.url ? 'ring-2 ring-indigo-600 scale-105 border-indigo-600' : 'border-gray-200 opacity-60 hover:opacity-100'
                      }`}
                    >
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text" required placeholder="Ej. Coca Cola 3L"
              ...
              {/* SKU con escáner */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Código de Barras / SKU *</label>
                <div className="flex gap-2">
                  <input
                    id="sku-scan-input"
                    type="text" required placeholder="Escanea o escribe..."
                  ...
                  <button
                    type="button"
                    onClick={() => (document.getElementById('sku-scan-input') as HTMLInputElement)?.focus()}
                    ...
                    title="Activar campo para lector"
                  >
                    <ScanBarcode className="w-5 h-5" />
                  </button>
                </div>
                <p className="text-[11px] text-gray-400">💡 Con lector físico: apunta al código y escanea directamente en el campo.</p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <select
                  className="w-full bg-gray-50 border border-gray-100 rounded-xl px-3 py-3 text-base focus:ring-2 focus:ring-indigo-600/10 focus:border-indigo-600 focus:bg-white outline-none"
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <div key={key} className="space-y-1">
                    ...
                    <input
                      type={type} step={step} required min="0"
